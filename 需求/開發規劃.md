...
2. **Phase 0.2: Docker 容器與環境變數 (.env) 配置**
   * 編寫 `docker/local/compose.yaml`，包含 Node.js (熱重載掛載)、PostgreSQL 16、Redis 7.2 容器。
   * 建立 `env/local/` 與 `.env.example`，設定資料庫連線、JWT Secret、Redis 端口。
3. **Phase 0.3: 資料庫 Drizzle ORM 與 Migration 設定**
...
* **手動驗證**:
  * 執行 `docker compose -f docker/local/compose.yaml up`，確認容器全部健康運行。
