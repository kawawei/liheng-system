...
graph LR
    ...
    P3 --> P4[Phase 4: WBS專案與Tab]:::level3
...
### Phase 4: 專案管理 5 大階段與立案協同模組 (WBS, Milestones & QA)

#### 4.1 階段目標
實現專案案號自動發號 (`PJ`)、5 大生命週期階段看板、簽約立案工期自動試算結案日、未稅/含稅/加5%營業稅精算、多階段雙向比例金額換算、需求追加變更單、URL 標籤頁狀態保持、工程師進度回報日誌與 Bug 缺陷監控。

#### 4.2 子階段任務細項
1. **Phase 4.1: 專案後端核心 API**
   * 實作專案 CRUD、工期與時程試算、多階段付款期程 API、需求追加變更單 API、進度日誌 API、QA Bug 缺陷 API。
   * 實作線上系統健康狀態更新 (正常/警告/異常)。
2. **Phase 4.2: 前端專案 5 大階段看板與清單 (`/projects`)**
   * 支援 5 階段看板與清單 (開發中 $\rightarrow$ 測試驗證 $\rightarrow$ 交付驗收 $\rightarrow$ 保固維護 $\rightarrow$ 正式結案)。
   * 支援一鍵簽約立案彈窗 (`ProjectCreateModal`)，提供工期結案日試算與計稅多階段試算。
3. **Phase 4.3: 專案核心詳情頁與 URL 標籤頁 Hook (`/projects/:id`)**
   * 封裝 `useUrlTabs.ts`，6 大 Tab 頁籤 (`milestones`, `logs`, `qa`, `change_orders`, `finance`, `line_sync`) 與 URL 參數雙向綁定。
