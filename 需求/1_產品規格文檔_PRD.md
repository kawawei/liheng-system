...
| :---: | :---: | :---: | :--- | :---: |
| v1.0.0 | 2026-08-14 | PM / Arch Team | 初稿建立：整合 CRM 客戶、合約報價、PMS 專案、多階段收支毛利、LINE 專案群組 AI 雙向協同、全局向量語意檢索 (pgvector)。 | 待審核 |
...
2. **專案研發透明度**: 7 大生命週期階段、里程碑進度表、工程師定期回報日誌、Bug 測試與運行狀況監控。
...
* **第一階段包含 (In-Scope - Phase 1)**:
  ...
  * [PMS] 專案 7 大階段、工程師指派、里程碑進度、日誌回報、測試與運行健康度、專案單號 (`PJ-YYYYMMDD-XXXX`)。
...
graph LR
    ...
    Dashboard --> PMS[專案研發管理]:::level2
    PMS --> ProjectList[專案看板與進度表]:::level3
    PMS --> ProjectDetail[專案詳情核心]:::level3
...
* **專案自動建案連動**: 合約狀態變更為已簽署時，系統提示一鍵建立對應之專案 (PMS) 並自動代入客戶名稱、合約金額與多階段收款期程。
...
| 專案管理 (PMS): 建立專案 / 階段變更 / 指派工程師 | 完整權限 | 僅檢視被指派專案 |
