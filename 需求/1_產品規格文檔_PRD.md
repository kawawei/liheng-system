...
graph LR
    ...
    S1[1. 開發階段]:::level1 --> S2[2. 測試驗證]:::level2
    S2 --> S3[3. 交付驗收]:::level2
    S3 --> S4[4. 保固維護]:::level3
    S4 --> S5[5. 正式結案]:::level3
```
...
* **一鍵立案與工期自動試算**:
  * 開始日期預設為立案當日，輸入預估工期天數自動計算「預計結案日期 (`開始日 + 工期天數`)」。
  * 支援延遲與提前動態監控 (提前交付 / 正常 / 延遲 +N 天)。
* **計稅模式與多階段付款**:
  * 含稅模式：原始金額即含稅總額，自動反推未稅金額與 5% 稅額（無外加 5% 選項）。
  * 未稅模式：原始金額為未稅金額，可勾選外加 5% 營業稅，自動試算稅額與含稅總額。
  * 動態新增階段：支援自由增減請款期數，提供「% 數」與「金額」雙向即時試算換算。
* **需求追加與變更單機制 (Change Orders)**:
  * 支援開立變更單 (`CO-YYYYMMDD-XXXX`)，記錄追加功能、追加金額 (未稅/含稅) 與追加工期。
  * 核准後自動連動專案最新總金額與最新預計結案日期。
* **里程碑與進度管理 (Milestones)**: 專案可自訂多個里程碑節點，標記完成進度百分比 (0% ~ 100%)。
* **工程師進度回報日誌 (Progress Log)**: 工程師填寫進度日誌（已完成項目、進行中項目、時程/技術阻礙）。
* **測試與線上運行狀況 (QA & Health Status)**:
  * 測試狀況: Bug 清單、缺陷等級 (Critical / Major / Minor)、修復狀態、驗收通過率。
  * 運行狀況: 線上系統健康狀態 (正常 / 警告 / 異常)、異常事件紀錄日誌。
...
| 專案管理 (WBS): 建立專案 / 階段變更 / 指派負責人 | 完整權限 | 僅檢視被指派專案 |
